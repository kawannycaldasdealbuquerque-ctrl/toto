...
$script:API_TOKEN = 'Qea9nvTMlaxN5YAXlfAyTbvlBttTbEzcA5qbFiIo3Mh5Dw28Z5vGPcq4CFwi'
$script:BASE_URL = 'https://api.invictuspay.app.br/api'
$script:AMOUNTS = @(3000, 5000, 10000, 20000, 30000, 50000, 100000, 300000)
...
function Invoke-InvictusRequest {
    ...
    if ($Uri.IndexOf('?') -ne -1) {
        $Uri = "$Uri&api_token=$script:API_TOKEN"
    } else {
        $Uri = "$Uri?api_token=$script:API_TOKEN"
    }
...
Write-Host "--- Starting Invictus Setup (Fixed Scope) ---"
...
foreach ($Amount in $script:AMOUNTS) {
